function getBlobData(live,budding,dead,total){
      
    // var blobData = getElementById("blobData")
    ...
    for (let i = 0; i < second_contours.size(); ++i) {
        ...
        $("#blobData").append( area + " " + circularity + " " + inertia + " " + convexity + ",");
    }
    ...
    maxCircularity = Math.max(circularities)
    minCircularity = Math.min(circularities)
    medianCircularity = myMedian(circularities)
    stDevCircularity = myStandardDeviation(circularities)  

    // console.log(circularities)
    console.log(" Max Circularity: " + maxCircularity)
    console.log(" Min Circularity: " + maxCircularity)
    console.log(" Median Circularity: " + medianCircularity)
    console.log(" StDev Circularity: " + stDevCircularity)
    
    //Get Median Inertia and Standard Deviation of Inertia
    maxInertia = Math.max(inertias)
    minInertia = Math.min(inertias)
    medianInertia = myMedian(inertias)
    stDevInertia = myStandardDeviation(inertias)  

    console.log(" Max Inertia: " + maxInertia)
    console.log(" Min Inertia: " + minInertia)
    console.log(" Median Inertia: " + medianInertia)
    console.log(" StDev Inertia: " + stDevInertia)
    
    //Get Median Convexity and Standard Deviation of Convexity
    maxConvexity = Math.max(convexities)
    minConvexity = Math.min(convexities)
    medianConvexity = myMedian(convexities)
    stDevConvexity = myStandardDeviation(convexities)  

    console.log(" Max Convexity: " + maxConvexity)
    console.log(" Min Convexity: " + minConvexity)
    console.log(" Median Convexity: " + medianConvexity)
    console.log(" StDev Convexity: " + stDevConvexity)
    ...
    return [medianCircularity,stDevCircularity,medianInertia,stDevInertia,medianConvexity,stDevConvexity]
    }
